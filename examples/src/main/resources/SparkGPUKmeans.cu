...
extern "C"
__global__ void getClusterCentroidsMod(int n, double *xs, double *norm, int *cluster_index, double *c, double *c_norm, int k, int d){
 //xs indicates datapoints, c indicates initial centroids, k indicates no. of clusters; d - dimensions

    extern __shared__ double local_means[];

    for (int i = threadIdx.x, kd = k * d; i < kd; i += blockDim.x)
    {
        local_means[i] = c[i];
    }
...
extern "C"
__global__ void calculateIntermediatesMod(int n, double *xs, int *cluster_index,
   int *intermediates0, double *intermediates1, int k, int d){
...
extern "C"
__global__ void calculateFinalMod(int n, int *intermediates0, double *intermediates1,
    int *s0, double *s1, int k, int d){
   ...
   if (blockIdx.x > 0) return;
        ...
        for (int clust = threadIdx.y; clust < k; clust+= blockDim.y){
                ...
                for (int dim = threadIdx.x; dim < d; dim+= blockDim.x) {
                        ...
                        // for (; s1cur < s1end; s1cur += kd, s2cur += kd)
                        for (; s1cur < s1end; s1cur += kd)
                        {
                                s1[start] += *s1cur;
 //                               s2[start] += *s2cur * *s2cur;
                        }
                }
        }
}
