...
object Demo2 {
  ...
  def main(args: Array[String]) {
    ...
    val sparkConf = new SparkConf().setAppName("SparkGPULR").setMaster(masterURL)
     ...
     // .set("spark.driver.extraJavaOptions","-Xmx25g -Xmn28g ")
    val sc = new SparkContext(sparkConf)
    val numSlices = if (args.length > 1) args(1).toInt else 80
    val N = if (args.length > 2) args(2).toInt else 1000000  // Number of data points
    val D = if (args.length > 3) args(3).toInt else 400   // Numer of dimensions
    val ITERATIONS = if (args.length > 4) args(4).toInt else 10
    val cpugpu = if (args.length > 5 ) args(5).toInt else 2 // 2 - both, 1 - gpu, 0 - cpu
    ...
    printf("numSlices=%d, Training Data=%d rows, Features=%d, Iterations=%d\n", numSlices, N, D, ITERATIONS)
    points.cacheGpu()
   
    if ( cpugpu != 0 ) {
    println("Load Data into GPU ... ")
    ...
    for (i <- 1 to ITERATIONS) {
      ...
      print("GPU iteration " + i)
      println(" - Elapsed time: %d ms".format(ms))
    }
    val ms123 = (System.nanoTime - now123) / 1000000
    println("Total Elapsed time: %d ms".format(ms123))
    ...
    for (i <- 1 to ITERATIONS) {
      ...
      print("CPU iteration " + i)
      val ms2 = (System.nanoTime - now2) / 1000000
      println("- Elapsed time: %d ms".format(ms2))
    }
    val ms212 = (System.nanoTime - now212) / 1000000
    println("Total Elapsed time: %d ms".format(ms212))
